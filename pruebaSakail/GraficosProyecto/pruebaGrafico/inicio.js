...
function calcularPorcentaje(veces_ingresado, total) {
    return ((veces_ingresado / total) * 100).toFixed(2) + '%';
}
...
function dibujarGrafico(datos) {
    ...
    // Calcular el total de veces ingresado
    const totalVecesIngresado = datos.reduce((total, item) => total + parseInt(item.veces_ingresado), 0);
    ...
    datos.forEach((item, indice) => {
        const porcentaje = parseFloat(item.porcentaje_ingreso);
        const anchoBarras = porcentaje * escala;
        ...
        const gradiente = contexto.createLinearGradient(x, y, x, y + alturaBarras);
        ...
        const animarBarra = () => {
            if (anchoBarra < anchoBarras) {
                ...
                // Siempre dibujar el porcentaje fuera de la barra
                contexto.fillStyle = '#000000'; // Cambiado a negro para los números fuera de las barras
                contexto.font = 'bold 18px Arial';
                contexto.textAlign = 'left';
                contexto.textBaseline = 'middle';
                contexto.fillText(item.porcentaje_ingreso, x + anchoBarras + 5, y + alturaBarras / 2);
            }
        };
        animarBarra();
